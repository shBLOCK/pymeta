...
pub(crate) fn execute(exe: PyMetaExecutable) -> PyMetaExecutionResult {
    ...
    let result = Python::attach(|py| -> PyResult<TokenStream> {
        ...
        let tokens = pymeta
            .call_method0("Tokens")
            .expect("Failed to construct Tokens");
        tokens
            .call_method0("__enter__")
            .expect("Tokens.__enter__() failed");
        ...
        tokens
            .call_method0("__exit__")
            .expect("Tokens.__exit__() failed");
...
mod _pymeta {
        ...
        fn inner_mut(&mut self) -> PyResult<&mut TokenStream> {
            match self.0 {
                ...
                None => Err(PyValueError::new_err(
                    "This TokenStream has already been consumed",
                )),
            }
        }
    ...
    impl PyTokenStream {
        ...
        fn append_group(
            ...
            let group = Group::new(
                delimiter,
                tokens.0.take().ok_or(PyValueError::new_err(
                    "The given TokenStream has already been consumed",
                ))?,
            );
        ...
        fn append_ident(&mut self, string: &str, span: Option<&mut PySpan>) -> PyResult<()> {
            let ident = Ident::new(
                string,
                span.map(|s| s.0.inner()).unwrap_or_else(Span::call_site),
            );
            ...
            let literal = match suffix {
                ...
                None => {
                    if let Ok(value) = value.extract::<i128>() {
                        ...
                        return Err(PyValueError::new_err(format!(
                            "Int literal value overflow: {value}"
                        )));
                    }
                }
            };
        ...
        fn append_float_literal(
            ...
            let literal = match suffix {
                ...
                None => {
                    ...
                    if !value.is_finite() {
                        return Err(PyValueError::new_err(format!(
                            "Invalid float literal value: {value}"
                        )));
                    }
